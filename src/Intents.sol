...
contract Intents {
    ...
    function encodeOnReceivedIntent(
        ...
        return
            bytes.concat(
                ...
                abi.encode(order, orderId, dataId)
            );
    }
    ...
    /// Returns ABI-encoded calldata of `onReceivedIntent(...)`.
    function encodeOnFulfillIntent(
        bytes32 orderId,
        bytes memory bundleRes
    ) private pure returns (bytes memory) {
        return
            bytes.concat(
                this.onFulfillIntent.selector,
                abi.encode(orderId, bundleRes)
            );
    }
    ...
    function fulfillIntent(
        ...
        for (uint8 i = 0; i < NUM_TARGET_BLOCKS; i++) {
            ...
            if (egp > 1000000) {
                ...
                require(
                    ...
                    bytes32(bundleRes) ==
                        0x7b226964223a312c22726573756c74223a7b2262756e646c6548617368223a22,
                    "bundle failed"
                );
            }
        }
        ...
        suaveCallData = encodeOnFulfillIntent(
            orderId,
            abi.encode(bundleRes, egps)
        );
    }
}
